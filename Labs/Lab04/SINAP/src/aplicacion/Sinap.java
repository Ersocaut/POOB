...
public class Sinap{
	...
	if ("".equals(nombre)){
		throw new SinapExcepcion(SinapExcepcion.NOMBRE_BLANCO);
	}
	if ("".equals(name)){
		throw new SinapExcepcion(SinapExcepcion.NAME_BLANCO);
	}
	if ("".equals(ubicación)){
		throw new SinapExcepcion(SinapExcepcion.UBICACION_BLANCO);
	}
	try{
		int narea = Integer.parseInt(area);
	}
	catch (Excepcion e ){
		throw new SinapExcepcion(SinapExcepcion.AREA_INTEGER);
	}
	if ("".equals(descripcion)){
		throw new SinapExcepcion(SinapExcepcion.);
	}
	try{
